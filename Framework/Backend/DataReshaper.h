...
public:
    ...
    static bool reshapeImage(PixelBufferDescriptor* dst, PixelDataType srcType,
        ...
        uint8_t* dstBytes = (uint8_t*) dst->buffer_;
        const int dstBytesPerRow = PixelBufferDescriptor::ComputeDataSize(dst->format_, dst->type_,
                dst->stride_ ? dst->stride_ : width, 1, dst->alignment_);
