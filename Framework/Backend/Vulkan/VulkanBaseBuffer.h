...
#ifndef OUR_GRAPHIC_FRAMEWORK_BACKEND_VULKAN_VULKANBASEBUFFER_H_
#define OUR_GRAPHIC_FRAMEWORK_BACKEND_VULKAN_VULKANBASEBUFFER_H_
#include <memory>
#include "../include_internal/IBuffer.h"
#include "VulkanDef.h"

namespace our_graph {
class VulkanBaseBuffer : public IBuffer{
 public:
  /**
   * 以创建的形式生成buffer
   * 该Buffer的memory以及实例自动创建
   * */
  explicit VulkanBaseBuffer(const std::string& name,
                               VkDevice device,
 ...
 protected:
  ...
  // 该变量描述了该buffer的信息，为BufferView的创建提供必要信息
  VkDescriptorBufferInfo buffer_descriptor_info_;
  std::string name_;
...
#endif //OUR_GRAPHIC_FRAMEWORK_BACKEND_VULKAN_VULKANBASEBUFFER_H_
